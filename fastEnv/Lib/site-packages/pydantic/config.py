...
class ConfigDict(TypedDict, total=False):
    ...
    - `'ignore'`: Providing extra data is ignored (the default):
      ...
      user = UserBase(name='John Doe', age=20)  # (2)!
      print(user)
      #> name='John Doe'
      ```
    ...
    class UserBase(BaseModel, validate_assignment=True):  # (1)!
        name: str

    user = UserBase(name='John Doe')  # (2)!
    print(user)
    #> name='John Doe'
    try:
        user.name = 123  # (3)!
        ...
        print(e)
        '''
        1 validation error for UserBase
        name
    ...
    class UserBase(BaseModel, revalidate_instances='never'):  # (1)!
        hobbies: list[str]
    ...
    my_user = UserBase(hobbies=['reading'])
    t = Transaction(user=my_user)
    print(t)
    #> user=UserBase(hobbies=['reading'])

    my_user.hobbies = [1]  # (2)!
    t = Transaction(user=my_user)  # (3)!
    print(t)
    #> user=UserBase(hobbies=[1])
    ...
    class UserBase(BaseModel, revalidate_instances='always'):  # (1)!
        hobbies: list[str]
    ...
    t = Transaction(user=my_sub_user)
    print(t)  # (3)!
    #> user=UserBase(hobbies=['scuba diving'])
    ```
    ...
    class UserBase(BaseModel, revalidate_instances='subclass-instances'):  # (1)!
        hobbies: list[str]
    ...
    2. This is not revalidated, since `my_user` is not a subclass of `UserBase`.
    3. Using `'never'` we would have gotten `user=SubUser(hobbies=['scuba diving'], sins=['lying'])`.
    """
