...
mapping which relates a ``UserBase`` to a ``SavingsAccount``::
    ...
    class SavingsAccount(Base):
        ...
        owner: Mapped[UserBase] = relationship(back_populates="accounts")
...
.. tip:: The ``UserBase.balance`` getter in the above example accesses the
   ``self.acccounts`` collection, which will normally be loaded via the
   :func:`.selectinload` loader strategy configured on the ``UserBase.balance``
...
At the expression level, it's expected that the ``UserBase`` class will
...
.. sourcecode:: pycon+sql
    ...
    >>> from sqlalchemy import select
    >>> print(
    ...     select(UserBase, UserBase.balance)
    ...     .join(UserBase.accounts)
    ...     .filter(UserBase.balance > 5000)
    ... )
...
.. sourcecode:: pycon+sql
    ...
    >>> print(
    ...     select(UserBase, UserBase.balance)
    ...     .outerjoin(UserBase.accounts)
    ...     .filter(or_(UserBase.balance < 5000, UserBase.balance == None))
    ... )
    {printsql}SELECT "user".id AS user_id, "user".name AS user_name,
    account.balance AS account_balance
...
.. sourcecode:: pycon+sql
    ...
    >>> from sqlalchemy import select
    >>> print(select(UserBase).filter(UserBase.balance > 400))
