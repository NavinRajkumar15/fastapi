...
class AssociationProxy(
    ...
    def for_class(
        ...
        target class for this proxy as mapped by ``UserBase``::

            inspect(UserBase).all_orm_descriptors["keywords"].for_class(UserBase).target_class

        This returns an instance of :class:`.AssociationProxyInstance` that
        is specific to the ``UserBase`` class.   The :class:`.AssociationProxy`
...
class AssociationProxyInstance(SQLORMOperations[_T]):
    ...
    :meth:`.AssociationProxy.for_class` method::
        ...
        proxy_state = inspect(UserBase).all_orm_descriptors["keywords"].for_class(UserBase)
