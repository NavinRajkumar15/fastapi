...
asking it to reflect the schema and produce mappings::
    ...
    UserBase = Base.classes.user
    Address = Base.classes.address

    session = Session(engine)

    # rudimentary relationships are produced
    session.add(Address(email_address="foo@bar.com", user=UserBase(name="foo")))
    session.commit()

    # collection-based relationships are by default named
    # "<classname>_collection"
    u1 = session.query(UserBase).first()
...
our basic mapping consisting of related ``UserBase`` and ``Address`` classes is
...
``user`` and ``address``, we can define one or both of the classes to be used::
    ...
    # pre-declare UserBase for the 'user' table
    class UserBase(Base):
        __tablename__ = "user"
    ...
    # but UserBase is the same as Base.classes.UserBase now
...
``UserBase.address_collection`` and the name of the class referred to, from
...
From the above mapping, we would now have classes ``UserBase`` and ``Address``,
where the collection from ``UserBase`` to ``Address`` is called
``UserBase.addresses``::

    UserBase, Address = Base.classes.UserBase, Base.classes.Address

    u1 = UserBase(addresses=[Address(email="foo@bar.com")])
...
defines table metadata::
    ...
    a1 = Address(email="u1")
    a2 = Address(email="u2")
    u1 = UserBase(address_collection=[a1, a2])
    assert a1.user is u1

Above, given mostly complete ``UserBase`` and ``Address`` mappings, the
:class:`_schema.ForeignKey` which we defined on ``Address.user_id`` allowed a
bidirectional relationship pair ``Address.user`` and
``UserBase.address_collection`` to be generated on the mapped classes.
...
class AutomapBase:
    ...
    :attr:`.AutomapBase.by_module` looks like::
        ...
        UserBase = Base.by_module.sqlalchemy.ext.automap.UserBase
