...
class Query(
    ...
    def __init__(
        ...
        E.g.::
            ...
            q = Query([UserBase, Address], session=some_session)

        The above is equivalent to::

            q = some_session.query(UserBase, Address)
    ...
    def get(self, ident: _PKIdentityArgument) -> Optional[Any]:
        ...
        E.g.::
            ...
            my_user = session.query(UserBase).get(5)
    ...
    def with_entities(
        ...
        e.g.::
            ...
            q = (
                ...
                .join(UserBase.address)
                .filter(UserBase.name.like("%ed%"))
                .order_by(Address.email)
            )

            # given *only* UserBase.id==5, Address.email, and 'q', what
            # would the *next* UserBase in the result be ?
            subq = (
                q.with_entities(Address.email)
                .order_by(None)
                .filter(UserBase.id == 5)
                .subquery()
            )
        ...
        *,
        ...
        E.g.::
            ...
            q = (
                sess.query(UserBase)
                .populate_existing()
                .with_for_update(nowait=True, of=UserBase)
            )
    ...
    def _filter_by_zero(self) -> Any:
        ...
        # to query(), e.g. query(UserBase).from_self() is still in terms of UserBase,
    ...
    def having(self, *having: _ColumnExpressionArgument[bool]) -> Self:
        ...
        functions like COUNT, SUM, AVG, MAX, and MIN, eg.::
            ...
            q = (
                session.query(UserBase.id)
                .join(UserBase.addresses)
                .group_by(UserBase.id)
    ...
    def join(
        ...
        **Simple Relationship Joins**
        ...
        Consider a mapping between two classes ``UserBase`` and ``Address``,
        with a relationship ``UserBase.addresses`` representing a collection
        of ``Address`` objects associated with each ``UserBase``.   The most
        common usage of :meth:`_query.Query.join`
        is to create a JOIN along this
        relationship, using the ``UserBase.addresses`` attribute as an indicator
        for how this should occur::

            q = session.query(UserBase).join(UserBase.addresses)

        Where above, the call to :meth:`_query.Query.join` along
        ``UserBase.addresses`` will result in SQL approximately equivalent to:
        ...
        In the above example we refer to ``UserBase.addresses`` as passed to
        ...
        .. note:: as seen in the above example, **the order in which each
           ...
           ``UserBase``, then ``Item``, then ``Order``, in our chain of joins; in
        ...
        **Joins to a Target Entity or Selectable**
        ...
        natural foreign key relationship between two entities::
            ...
            q = session.query(UserBase).join(Address)
        ...
        **Joins to a Target with an ON Clause**
        ...
        a SQL expression as the ON clause is as follows::
            ...
            q = session.query(UserBase).join(Address, UserBase.id == Address.user_id)

        The above form may also use a relationship-bound attribute as the
        ON clause as well::

            q = session.query(UserBase).join(Address, UserBase.addresses)
        ...
        aliases set up using the :func:`~sqlalchemy.orm.aliased` function::
            ...
            q = (
                session.query(UserBase)
                .join(a1, UserBase.addresses)
                .join(a2, UserBase.addresses)
        ...
        equivalent to the one above would be::
            ...
            q = (
                session.query(UserBase)
                .join(UserBase.addresses.of_type(a1))
                .join(UserBase.addresses.of_type(a2))
                .filter(a1.email_address == "ed@foo.com")
                .filter(a2.email_address == "ed@bar.com")
            )
        ...
        **Augmenting Built-in ON Clauses**
        ...
        with the default criteria using AND::
            ...
            q = session.query(UserBase).join(
                UserBase.addresses.and_(Address.email_address != "foo@bar.com")
            )
        ...
        **Joining to Tables and Subqueries**
        ...
        out of a query::
            ...
            q = session.query(UserBase).join(subq, UserBase.id == subq.c.user_id)
        ...
        entity using :func:`_orm.aliased`::
            ...
            q = session.query(UserBase).join(UserBase.addresses.of_type(address_subq))
    ...
    def slice(
        ...
        For example, ::
            ...
            session.query(UserBase).order_by(UserBase.id).slice(1, 3)
    ...
    def column_descriptions(self) -> List[ORMColumnDescription]:
        ...
        Format is a list of dictionaries::
            ...
            user_alias = aliased(UserBase, name="user2")
            q = sess.query(UserBase, UserBase.id, user_alias)
    ...
    def exists(self) -> Exists:
        ...
        e.g.::
            ...
            q = session.query(UserBase).filter(UserBase.name == "fred")
        ...
        The EXISTS construct is usually used in the WHERE clause::
            ...
            session.query(UserBase.id).filter(q.exists()).scalar()
    ...
    def count(self) -> int:
        ...
        expressions in conjunction with :meth:`~.Session.query`, i.e.::
            ...
            # count UserBase records, without
            # using a subquery.
            session.query(func.count(UserBase.id))

            # return count of user "id" grouped
            # by "name"
            session.query(func.count(UserBase.id)).group_by(UserBase.name)

            from sqlalchemy import distinct

            # count distinct "name" values
            session.query(func.count(distinct(UserBase.name)))
    ...
    def delete(
        ...
        E.g.::
            ...
            sess.query(UserBase).filter(UserBase.age == 25).delete(synchronize_session=False)

            sess.query(UserBase).filter(UserBase.age == 25).delete(
    ...
    def update(
        ...
        E.g.::
            ...
            sess.query(UserBase).filter(UserBase.age == 25).update(
                {UserBase.age: UserBase.age - 10}, synchronize_session=False
            )

            sess.query(UserBase).filter(UserBase.age == 25).update(
                {"age": UserBase.age - 10}, synchronize_session="evaluate"
            )
