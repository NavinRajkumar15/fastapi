...
class AliasedClass(
    ...
    mapped entity can be used in multiple contexts.   A simple example::
        ...
        user_alias = aliased(UserBase)
        session.query(UserBase, user_alias).join(
            (user_alias, UserBase.id > user_alias.id)
        ).filter(UserBase.name == user_alias.name)
...
def with_parent(
    ...
    E.g.::
        ...
        stmt = select(Address).where(with_parent(some_user, UserBase.addresses))
    ...
    to indicate the left side of the criteria::
        ...
        a1 = aliased(Address)
        a2 = aliased(Address)
        stmt = select(a1, a2).where(with_parent(u1, UserBase.addresses.of_type(a2)))
    ...
    :func:`_orm.with_parent.from_entity` argument::
        ...
        stmt = select(a1, a2).where(
            with_parent(u1, UserBase.addresses, from_entity=a2)
        )
