...
class _AbstractLoad(traversals.GenerativeOnTraversal, LoaderOption):
    ...
    def contains_eager(
        ...
        ``UserBase`` entity, and the returned ``Order`` objects would have the
        ...
        primary collection of parent objects may already have been loaded::
            ...
            sess.query(UserBase).join(UserBase.addresses).filter(
                Address.email_address.like("%@aol.com")
            ).options(contains_eager(UserBase.addresses)).populate_existing()
    ...
    def load_only(self, *attrs: _AttrType, raiseload: bool = False) -> Self:
        ...
        Example - given a class ``UserBase``, load only the ``name`` and
        ``fullname`` attributes::

            session.query(UserBase).options(load_only(UserBase.name, UserBase.fullname))

        Example - given a relationship ``UserBase.addresses -> Address``, specify
        subquery loading for the ``UserBase.addresses`` collection, but on each
        ``Address`` object load only the ``email_address`` attribute::

            session.query(UserBase).options(
                subqueryload(UserBase.addresses).load_only(Address.email_address)
            )
        ...
        specifically referred to using the :class:`_orm.Load` constructor::
            ...
            stmt = (
                select(UserBase, Address)
                .join(UserBase.addresses)
                .options(
                    Load(UserBase).load_only(UserBase.name, UserBase.fullname),
    ...
    def joinedload(
        ...
        examples::
            ...
            # joined-load the "orders" collection on "UserBase"
            select(UserBase).options(joinedload(UserBase.orders))
    ...
    def subqueryload(self, attr: _AttrType) -> Self:
        ...
        examples::
            ...
            # subquery-load the "orders" collection on "UserBase"
            select(UserBase).options(subqueryload(UserBase.orders))
    ...
    def selectinload(
        ...
        examples::
            ...
            # selectin-load the "orders" collection on "UserBase"
            select(UserBase).options(selectinload(UserBase.orders))
...
class _LoadElement(
    ...
    def _adjust_effective_path_for_current_path(
        ...
        .. sourcecode:: text
            ...
            lp: UserBase -> orders -> Order -> items -> Item -> keywords -> Keyword

            cp: UserBase -> orders -> Order -> items
...
class _AttributeStrategyLoad(_LoadElement):
    ...
    e.g.::
        ...
        joinedload(UserBase.addresses)
        defer(Order.name)
        selectinload(UserBase.orders).lazyload(Order.items)
...
class _TokenStrategyLoad(_LoadElement):
    ...
    e.g.::
        ...
        raiseload("*")
        Load(UserBase).lazyload("*")
        defer("*")
        load_only(UserBase.name, UserBase.email)  # will create a defer('*')
        joinedload(UserBase.addresses).raiseload("*")
