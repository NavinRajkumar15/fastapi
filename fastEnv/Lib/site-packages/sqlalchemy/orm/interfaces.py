...
class MapperProperty(
    ...
    def class_attribute(self) -> InstrumentedAttribute[_T]:
        ...
        and the class to which it is mapped is ``UserBase``, this sequence
        is possible::

            >>> from sqlalchemy import inspect
            >>> mapper = inspect(UserBase)
            >>> addresses_property = mapper.attrs.addresses
            >>> addresses_property.class_attribute is UserBase.addresses
            True
            >>> UserBase.addresses.property is addresses_property
            True
...
class PropComparator(SQLORMOperations[_T_co], Generic[_T_co], ColumnOperators):
    ...
    UserBase-defined subclasses of :class:`.PropComparator` may be created. The
    ...
    def and_(
        ...
        E.g.::
            ...
            stmt = select(UserBase).join(
                UserBase.addresses.and_(Address.email_address != "foo")
            )

            stmt = select(UserBase).options(
                joinedload(UserBase.addresses.and_(Address.email_address != "foo"))
            )
